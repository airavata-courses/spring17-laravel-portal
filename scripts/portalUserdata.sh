...
sudo docker run -d -e SERVICE_IGNORE -p 8400:8400 -p 8500:8500 -p 8300:8300 -p 8301:8301 -p 8301:8301/udp -p 8302:8302 -p 8302:8302/udp -p 8600:53/udp -h consul`wget -qO- http://instance-data/latest/meta-data/public-ipv4 | cut -d . -f 4` --name consul`wget -qO- http://instance-data/latest/meta-data/public-ipv4 | cut -d . -f 4` progrium/consul -server -advertise `wget -qO- http://instance-data/latest/meta-data/public-ipv4` -join $CONSUL_SERVER_IP >> /var/log/sga-consul.log 2>&1
